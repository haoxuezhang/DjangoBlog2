...
from .models import Author, Article, Comment, Poll, NewUser
from .forms import *
...
def article(request, article_id):
    ...
    content = markdown2.markdown(article.content,
                                 extras=["code-friendly", "fenced-code-blocks", "headers-ids", "toc", "metadata"])
...
def get_poll_article(request, article_id):
    logged_user = request.user
    article = Article.object.get(id=article_id)
    polls = logged_user.article_set.all()
    articles = []
    for poll in polls:
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('raw_username', 'erjgiqfv240hqp5668ej23foi') != 'erjgiqfv240hqp5668ej23foi':
            try:
                ...
                return render(request, 'register.html', {'form': form, 'mag': error1})
        else:
            if form.is_valid():
                ...
                if password1 != password2:
                    return render(request, 'register.html', {'form': form, 'msa': 'two password is different !'})
                else:
